...
schedule <- 
  schedule %>% 
  filter(Division == "D4" & !is.na(`Final Score` )) %>% 
  mutate(
...
schedule[schedule$Home == "Sonajita", ]$`Away Score`  <- 10

schedule$our_goals = 
  with(schedule, 
       case_when(
         Home == "Barons" ~ `Home Score`, 
         T ~ `Away Score`
       ))
schedule$our_conceded = 
  with(schedule, 
       case_when(
         Home == "Barons" ~ `Away Score`, 
         T ~ `Home Score`
       ))
...
Goals <- schedule %>% select(our_goals) %>% sum() %>% unlist()
Games <- schedule %>% filter(Division == "D4") %>% nrow() 
...
Goals <- schedule %>% filter(Division == "D4") %>% summarize(sum(our_conceded)) %>% unlist()
Games <- schedule %>% filter(Division == "D4") %>% nrow()
...
cap <- paste0("Goals Due to Sonajita's Forfeit")

flexdashboard::valueBox(value = 10, caption = cap, color = "orange", icon = 'fa-gear')
```
...
advanced_assist_tally %>% 
  kable(align = 'c', 
        col.names = c(
          "Assist Creator", "Assister Total", 
          "Assist Recipient", "Assists to Scorer", "% of Assister Total"
        )) %>% 
  kable_styling(bootstrap_options = c("striped", "condensed"))
```
...
assistless <- 
  ...
  mutate(
    p_total = paste0(round(Goal / Goals, 2) * 100, "%")
  ) %>% 
...
assistless %>% 
  kable(align = 'c', 
        col.names = c(
          "Player", "Assist-less Goals", "% Of Total Goals"
        )) %>% 
  kable_styling(bootstrap_options = c("striped", "condensed"))
...
advanced_assist_tally %>% 
  ...
  kable(align = 'c', 
        col.names = c(
          "Players in Duo", "Total Goals Scored "
        )) %>% 
  kable_styling(bootstrap_options = c("striped", "condensed"))
